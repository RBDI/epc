...
while ($row=mysql_fetch_array($result)) {
	...
	$items[$row['item_id']]['subitem'][$row['ID']]['zprice']=$row['value4'];
...
foreach ($items as $id => $item){
	...
	// $item['desc']=mb_substr($item['desc'], 0, 512);
		...
		// ';
			...
			$itm_ctg[$item['type']][$subitem_id][8]=$zprice;
...
foreach ($list as $key => $cat) {
	if ($itm_ctg[$cat[1]]){
		...
		foreach ($itm_ctg[$cat[1]] as $itm) {
			if ($itm[1]=='') $itm[1]='EPC-'.$itm[0];
			else $itm[1].='-'.$itm[0];
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$i, $itm[1]);
			$name=$brands[$itm[2]]['name'].' '.$itm[3].', '.$itm[6];
			if ($itm[5]!='') $name.=' ('.$itm[5].')';
			...
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$i, $itm[7]);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$i, $itm[8]);
